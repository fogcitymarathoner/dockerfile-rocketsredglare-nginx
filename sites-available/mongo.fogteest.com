    ...
    upstream mongo_tools_app {
         server 127.0.0.1:8104;
    }
    server {
       server_name www.mongo.fogtest.com;
       #rewrite ^(.*) http://www.sfgeek.net$1 permanent;

    return 301 $scheme://mongo.fogtest.com$request_uri;
    }
	...
	root /home/marc/python_apps/renesola;
        ...
        server_name  mongo.fogtest.com;

        access_log /var/log/nginx/mongo.fogtest.log;
        error_log /var/log/nginx/mongo.fogtest.log;

       location / {
          proxy_pass         http://mongo_tools_app;
          proxy_redirect     off;
    ...
    server {
        ...
        access_log /var/log/nginx/mongo.fogtest.access.log;
        error_log /var/log/nginx/mongo.fogtest.error.log;
